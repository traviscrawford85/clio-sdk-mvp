...
def collect_parameters(operation, path_item, root):
    ...
    params = []
    seen = set()
    for param_list in [path_item.get("parameters", []), operation.get("parameters", [])]:
...
def main():
    spec_path = Path("openapi/expanded_full.yaml")
    ...
    for path, path_item in spec.get("paths", {}).items():
        for method in path_item:
            ...
            if "requestBody" in operation:
                req_body_schema = get_request_body_schema_name(operation["requestBody"], root)
            # Response codes
