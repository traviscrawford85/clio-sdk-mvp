from collections.abc import Mapping
from typing import Any, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="EmailAddressBase")


@_attrs_define
class EmailAddressBase:
    """
    Attributes:
        address (Union[Unset, str]): The address of the *EmailAddress*
        created_at (Union[Unset, str]): The time the *EmailAddress* was created (as a ISO-8601 timestamp)
        etag (Union[Unset, str]): ETag for the *EmailAddress*
        id (Union[Unset, int]): Unique identifier for the *EmailAddress*
        name (Union[Unset, str]): A descriptive name for the *EmailAddress. Common values include `Home`, `Work`, and
            `Other`, as these are the only selectable options within Clio Manage, but other values may be returned for this
            field.
        primary (Union[Unset, bool]): Whether it is the default for this contact
        updated_at (Union[Unset, str]): The time the *EmailAddress* was last updated (as a ISO-8601 timestamp)
    """

    address: Union[Unset, str] = UNSET
    created_at: Union[Unset, str] = UNSET
    etag: Union[Unset, str] = UNSET
    id: Union[Unset, int] = UNSET
    name: Union[Unset, str] = UNSET
    primary: Union[Unset, bool] = UNSET
    updated_at: Union[Unset, str] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        address = self.address

        created_at = self.created_at

        etag = self.etag

        id = self.id

        name = self.name

        primary = self.primary

        updated_at = self.updated_at

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if address is not UNSET:
            field_dict["address"] = address
        if created_at is not UNSET:
            field_dict["created_at"] = created_at
        if etag is not UNSET:
            field_dict["etag"] = etag
        if id is not UNSET:
            field_dict["id"] = id
        if name is not UNSET:
            field_dict["name"] = name
        if primary is not UNSET:
            field_dict["primary"] = primary
        if updated_at is not UNSET:
            field_dict["updated_at"] = updated_at

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        address = d.pop("address", UNSET)

        created_at = d.pop("created_at", UNSET)

        etag = d.pop("etag", UNSET)

        id = d.pop("id", UNSET)

        name = d.pop("name", UNSET)

        primary = d.pop("primary", UNSET)

        updated_at = d.pop("updated_at", UNSET)

        email_address_base = cls(
            address=address,
            created_at=created_at,
            etag=etag,
            id=id,
            name=name,
            primary=primary,
            updated_at=updated_at,
        )

        email_address_base.additional_properties = d
        return email_address_base

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
